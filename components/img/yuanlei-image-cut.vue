	...
	export default {
		...
		methods: {
			...
			getImageInfo() {
				...
				ctx.drawImage(this.imageSrc, IMG_RATIO >= 1 ? 0 : (IMG_REAL_H - IMG_REAL_W) / 2, IMG_RATIO <= 1 ? 0 : (
					IMG_REAL_W - IMG_REAL_H) / 2, IMG_REAL_W, IMG_REAL_H)
				ctx.draw(true, () => {
					const pixelRatio = uni.getSystemInfoSync().pixelRatio || 2
					const cutWidth = 750 - this.cutL - this.cutR
					const cutHeight = 750 - this.cutT - this.cutB

					uni.canvasToTempFilePath({
						x: this.rpxToPx(this.cutL),
						y: this.rpxToPx(this.cutT),
						width: this.rpxToPx(cutWidth),
						height: this.rpxToPx(cutHeight),
						destWidth: this.rpxToPx(cutWidth) * pixelRatio * this.scaleTate,
						destHeight: this.rpxToPx(cutHeight) * pixelRatio * this.scaleTate,
			...
			dragMove(e) {
				...
				switch (dragType) {
					...
					case 'left':
						...
						if ((CUT_L - dragLengthL) > (SCREEN_WIDTH - this.cutR)) dragLengthL = CUT_L - (SCREEN_WIDTH - this
							.cutR)
						this.setData({
					...
					case 'top':
						...
						if ((CUT_T - dragLengthT) > (SCREEN_WIDTH - this.cutB)) dragLengthT = CUT_T - (SCREEN_WIDTH - this
							.cutB)
						this.setData({
